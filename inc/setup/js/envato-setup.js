jQuery(document).ready(function($){
        ...
        jQuery(document).on('click', '.style-demo-image', function(){
            jQuery('ul.list-styles li').removeClass('active');
            jQuery(this).closest('ul.list-styles li').addClass('active');
            $(this).addClass('current');
            var newcolor = $(this).find('a').data('style');
            $('#demo_style').val(newcolor);
            
        });

        jQuery('.zd-import-page').on('click', function(e){
            jQuery('#zd-res').children('.res-text').html('');
            jQuery('#zd-res').children('.res-text').html('Processing content...');
            jQuery('#zd-res').find('.loader_img').show();
            jQuery.ajax({
                type : "post",
                ...
                success: function(response) {
                    if(response){
                        jQuery('#zd-res').children('.res-text').html(response);
                        jQuery('#zd-res').find('.loader_img').hide();
                        jQuery('#zd-res').find('.done_img').show();
                    
                        jQuery('#zd-res-homepage').children('.res-text').html('');
                        jQuery('#zd-res-homepage').children('.res-text').html('Adding Homepage...');
                        jQuery('#zd-res-homepage').find('.loader_img').show();
                        ...
                        url : envato_setup_params.ajaxurl,
                        data : {
                            action:'demo_homepage',
                            data:'1',
                            },
                        
                            success: function(responseMen) {
                                if(responseMen){
                                    jQuery('#zd-res-homepage').children('.res-text').html(responseMen);
                                    jQuery('#zd-res-homepage').find('.loader_img').hide();
                                    jQuery('#zd-res-homepage').find('.done_img').show();
                        
                                    jQuery('#zd-res-themeoptions').children('.res-text').html('');
                                    jQuery('#zd-res-themeoptions').children('.res-text').html('Theme Options import...');
                                    jQuery('#zd-res-themeoptions').find('.loader_img').show();
                                    jQuery.ajax({
                                        type : "post",
                                        url : envato_setup_params.ajaxurl,
                                        data : {
                                            action:'demo_theme_options',
                                            data:'1',
                                            },
                                            
                                        success: function(responseMen) {
                                            if(responseMen){
                                                jQuery('#zd-res-themeoptions').children('.res-text').html(responseMen);
                                                jQuery('#zd-res-themeoptions').find('.loader_img').hide();
                                                jQuery('#zd-res-themeoptions').find('.done_img').show();
                                                jQuery( ".button-go-next" )[0].click();
                                            }
